...
export async function deployVaults(
  ...
  const l1VaultFactory: ContractFactory = await scriptUtils.getContractFactory("L1Vault");
  ...
  const l2VaultFactory: ContractFactory = await scriptUtils.getContractFactory("L2Vault");
  const l2Vault: Contract = await l2VaultFactory.deploy(
    governance,
    polygonUSDCAddress,
    9,
    1,
