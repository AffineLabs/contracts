...
export async function deployFxBridge(
  ...
  const FxStateRootTunnelFactory: ContractFactory = await scriptUtils.getContractFactory("FxStateRootTunnel");
  const fxStateRootTunnel: Contract = await FxStateRootTunnelFactory.deploy(
    checkpointManager,
    fxRootAddress,
  ...
  const FxStateChildTunnelFactory: ContractFactory = await scriptUtils.getContractFactory("FxStateChildTunnel");
  const fxStateChildTunnel: Contract = await FxStateChildTunnelFactory.deploy(
    fxChildAddress,
