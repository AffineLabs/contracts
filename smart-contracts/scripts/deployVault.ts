...
async function main() {
  ...
  console.log("Deploying contracts with the account:", [governance.address, tokenOwner.address, strategist.address]);

  // TestMintable by `tokenOwner`.
  const tokenFactory: ContractFactory = await ethers.getContractFactory("TestMintable", tokenOwner);
  const token: Contract = await tokenFactory.deploy(ethers.utils.parseUnits("100000", 6));
  ...
  // Deploy strategy
  const stratFactory = await ethers.getContractFactory("TestStrategy", strategist);
  ...
  const addTx = await vault
    .connect(governance)
    .addStrategy(strategy.address, 9000, 0, ethers.utils.parseUnits("100000", 6));
  await addTx.wait();
}
