...
for protocol in protocols_of_interest:
    ...
    written_file_path = write_to_file(
        jsonable_dict[protocol], file_name=protocol, save_dir=TEMP_LOCAL_SAVE_DIR
    )
    logging.info("Written ", written_file_path)
    success_cache = upload_to_s3(
        S3_BUCKET_FOR_API_DATA,
        written_file_path,
        "lending_protocol_data/" + f"{protocol}/{os.path.basename(written_file_path)}",
    )
    ...
    success_update = upload_to_s3(
        S3_BUCKET_FOR_API_DATA,
        written_file_path,
        "lending_protocol_data/" + f"{protocol}/latest.csv",
    )

    if not success_update:
        logging.warning(f"S3 upload of latest.csv failed for {protocol}")
