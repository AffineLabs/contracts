...
async def collect_data(url, address):
    ...
    async with Client(
        ...
        while True:
            ...
            logging.info(
                f"Collected {len(result['pairDayDatas'])} datapoints for contract {address}."
            )
...
async def main():
    ...
    for (addr, name, fee_tier), url in amm_pool_iterator:
        ...
        written_file_path = write_to_file(
            results, file_name=name, save_dir=TEMP_LOCAL_SAVE_DIR
        )
        ...
        success_update = upload_to_s3(
            S3_BUCKET_FOR_API_DATA,
            written_file_path,
            "amm_liquidity_pool/" + f"{name.split('-')[0]}/{name}/latest.csv",
        )
        if not success_update:
            logging.warning(
                f"S3 upload of latest.csv failed for pair {name}, address {addr}"
            )
