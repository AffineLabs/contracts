...
library Sslp {
    ...
    function deployPoly(AffineVault vault, LendingParams memory params) internal returns (DeltaNeutralLp strategy) {
    ...
    function deployEth(AffineVault vault, LendingParams memory params) internal returns (DeltaNeutralLp strategy) {
        strategy = new DeltaNeutralLp(
        vault,
        LenderInfo({
    ...
    function deployEthWeth(AffineVault vault, LendingParams memory params) internal returns (DeltaNeutralLp strategy) {
        strategy = new DeltaNeutralLp(
        vault,
        LenderInfo({
            pool: ILendingPool(0x7d2768dE32b0b80b7a3454c06BdAc94A69DDc7A9),
...
contract Deploy is Script {
    ...
    function _start() internal {
        ...
        params = LendingParams({assetToDepositRatioBps: 5714, collateralToBorrowRatioBps: 7500});
        vm.startBroadcast(deployer);
    }

    function runPoly() external {
        _start();
        Sslp.deployPoly(AffineVault(0x829363736a5A9080e05549Db6d1271f070a7e224), params);
    }

    function runEth() external {
        _start();
        Sslp.deployEth(AffineVault(0x84eF1F1A7f14A237c4b1DA8d13548123879FC3A9), params);
    }

    function runEthWethSushi() external {
        _start();
        DeltaNeutralLp strategy = Sslp.deployEthWeth(AffineVault(0x87964c5a8F2b13e04b86B4D12e0EeA13Ff144DC3), params);
