...
describe("I/O", async () => {
  it("Can deposit and withdraw", async () => {
    ...
    await token
      .connect(user)
      .approve(vault.address, ethers.utils.parseUnits("100", 6));
    await vault
      .connect(user)
      .deposit(user.address, ethers.utils.parseUnits("5", 6));
    ...
    expect(await token.balanceOf(user.address)).to.equal(
      ethers.utils.parseUnits("95", 6)
    );

    // withdrawing 1 share gets one usdc
    await vault
      .connect(user)
      .withdraw(user.address, ethers.utils.parseUnits("1", 6));
    expect(await vault.balanceOf(user.address)).to.equal(
      ethers.utils.parseUnits("4", 6)
    );
    expect(await token.balanceOf(user.address)).to.equal(
      ethers.utils.parseUnits("96", 6)
    );
    ...
    await token
      .connect(user)
      .transfer(vault.address, ethers.utils.parseUnits("4", 6));
    await vault
      .connect(user)
