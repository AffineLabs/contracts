...
export async function deployStagings(
  ...
  const creationCode = (await scriptUtils.getContractFactory("Staging")).bytecode;
  const encodedParams = ethers.utils.defaultAbiCoder
    .encode(
  ...
  const l1Staging: Contract = (await scriptUtils.getContractFactory("Staging")).attach(l1StagingAddress);
  ...
  const l2Staging: Contract = (await scriptUtils.getContractFactory("Staging")).attach(l2StagingAddress);
