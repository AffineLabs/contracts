...
abstract contract BaseStrategy {
    ...
    function divest(uint256 amount, DivestType divestType) external onlyVault returns (uint256, DivestResponse) {
        return _divest(amount, divestType);
    }

    /// @dev This function should not revert if we get less than `amount` out of the strategy
    function _divest(uint256 amount, DivestType divestType) internal virtual returns (uint256, DivestResponse) {}
